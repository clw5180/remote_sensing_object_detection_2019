...
def merge(bbox, angle=5, thres_width=1.2, thres_height=1.1, padding=1.0):
    if bbox.shape[0] == 0:
        return bbox
...
def merge_write(root):
    ...
    for _file in files:
        ...
        for line in lines:
            ...
            bbox.append([float(record[0]), float(record[1]), float(record[2]), float(record[3]), float(record[4]),
                         float(record[5])])
        im_l = _file.split(".")
